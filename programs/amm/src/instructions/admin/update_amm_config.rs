...
pub fn update_amm_config(ctx: Context<UpdateAmmConfig>, param: u8, value: u32) -> Result<()> {
    let amm_config = &mut ctx.accounts.amm_config.load_mut()?;
...
fn update_protocol_fee_rate(amm_config: &mut RefMut<'_, AmmConfig>, protocol_fee_rate: u32) {
    assert!(protocol_fee_rate <= FEE_RATE_DENOMINATOR_VALUE);
    assert!(protocol_fee_rate + amm_config.fund_fee_rate <= FEE_RATE_DENOMINATOR_VALUE);
    amm_config.protocol_fee_rate = protocol_fee_rate;
}

fn update_trade_fee_rate(amm_config: &mut RefMut<'_, AmmConfig>, trade_fee_rate: u32) {
    assert!(trade_fee_rate < FEE_RATE_DENOMINATOR_VALUE);
    amm_config.trade_fee_rate = trade_fee_rate;
}

fn update_fund_fee_rate(amm_config: &mut RefMut<'_, AmmConfig>, fund_fee_rate: u32) {
...
fn set_new_owner(amm_config: &mut RefMut<'_, AmmConfig>, new_owner: Pubkey) {
...
fn set_new_fund_owner(amm_config: &mut RefMut<'_, AmmConfig>, new_fund_owner: Pubkey) {
    #[cfg(feature = "enable-log")]
    msg!(
